...
Requires the environment variable MISTRAL_API_KEY with a valid key
MISTRAL was choosen over Google due to the lack diarize feature and excessive costs - Mistral 0.004 versus Google 0.013
"""
...
source_file_directory: Path = Path("/Users/anton.plesner/IdeaProjects/data/language_transfer_spanish/test")
transcription_file_directory: Path = Path("/Users/anton.plesner/IdeaProjects/data/language_transfer_spanish/transcriptions")
progress_file: Path = Path("/Users/anton.plesner/IdeaProjects/data/language_transfer_spanish/transcriptions/.progress.json")
...
def produce_transcription(filename: Path) -> Optional[object]:
    ...
    try:
        ...
        logger.info(f"Successfully transcribed {filename.name}")
...
def main():
    ...
    for filename in audio_files:
        ...
        if filename in state.failed_files:
            logger.warning(f"Skipping previously failed file: {filename}")
            continue
        
        logger.info(f"Processing {filename} ({processed_count + 1}/{total_files})...")
        
        try:
            transcription = produce_transcription(source_file_directory.joinpath(filename))
