...
def test_returns_firestore_value_when_active_prompt_exists() -> None:
    ...
    with patch("app.db.system_prompts.get_active", return_value={"prompt_text": "Firestore prompt"}):
...
def test_returns_default_on_exception() -> None:
    ...
    with patch("app.db.system_prompts.get_active", side_effect=RuntimeError("connection failed")):
        import app.agents.prompt_loader
